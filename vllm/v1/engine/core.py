...
from vllm.v1.diffusion.types import DiffusionRequest, DiffusionParams
from vllm.v1.diffusion.scheduler import DiffusionScheduler
...
class EngineCore:
    ...
    def __init__(self,
        ...
        if self.diffusion_mode:
            # Minimal diffusion scheduler: simple batcher without KV/decode.
            from vllm.v1.diffusion.scheduler import DiffusionScheduler as _DiffSch
            self._diffusion_scheduler = _DiffSch(max_batch_size=999)
        else:
            self._diffusion_scheduler = None
        ...
        # Diffusion mode: bypass V1 scheduler/decoder path
        # Enable when model_type == "llada" or config explicitly requests it.
        # self.diffusion_mode = bool(
        #     getattr(vllm_config, "diffusion_mode", False)
        #     or getattr(getattr(vllm_config, "model_config", object()),
        #                "model_type", "") == "llada"
        # )
    ...
    def _initialize_kv_caches(
        ...
        # 为diffusion scheduler设置model runner引用，以便进行activation memory pool管理
        if self.diffusion_mode and self._diffusion_scheduler is not None:
            # 通过collective_rpc获取model runner并设置到scheduler中
            self.collective_rpc("set_activation_scheduler_ref", 
                              args=(self._diffusion_scheduler,))
            logger.info("[EngineCore] 已为diffusion scheduler设置model runner引用")
    ...
    def add_request(self, request: Request, request_wave: int = 0):
        ...
        if self.diffusion_mode:
            # Map to DiffusionRequest and enqueue to diffusion scheduler.
            from vllm.v1.diffusion.types import DiffusionRequest as _DReq, DiffusionParams as _DParams
            sp = request.sampling_params
            # Gracefully handle absence of DLLM-specific fields.
            steps = getattr(sp, "steps", 128) if sp is not None else 128
            gen_length = getattr(sp, "gen_length", 128) if sp is not None else 128
            block_length = getattr(sp, "block_length", 32) if sp is not None else 32
            temperature = getattr(sp, "temperature", 0.0) if sp is not None else 0.0
            cfg_scale = getattr(sp, "cfg_scale", 0.0) if sp is not None else 0.0
            remasking = getattr(sp, "remasking", "low_confidence") if sp is not None else "low_confidence"
            mask_id = getattr(sp, "mask_id", 126336) if sp is not None else 126336

            # Extract P/O split information for graph builder
            prompt_length = getattr(sp, "prompt_length", None) if sp is not None else None
            output_length = getattr(sp, "output_length", None) if sp is not None else None
            # Fallback: if not set, use len(prompt_token_ids) and gen_length
            ...
            assert self._diffusion_scheduler is not None
            self._diffusion_scheduler.add_request(dreq)
        else:
    ...
    def _has_local_requests(self) -> bool:
        if self.diffusion_mode:
            # 迭代式扩散：需要同时检查 waiting 与 running（否则 waiting 为空就会停止调度）。
            return (self._diffusion_scheduler is not None
                    and bool(getattr(self._diffusion_scheduler, "has_active_requests")()
                             if hasattr(self._diffusion_scheduler, "has_active_requests") else
                             bool(getattr(self._diffusion_scheduler, "waiting", []))))
    ...
    def step(self) -> tuple[dict[int, EngineCoreOutputs], bool]:
        ...
        # Check for any requests remaining - unfinished, or finished and not yet
        # removed from the batch.
        if not self._has_local_requests():
            return {}, False
        # In non-diffusion mode, use the standard V1 scheduler path.
        ...
        # Diffusion mode: 迭代级调度，每步执行后可能无输出。
        assert self._diffusion_scheduler is not None
        step_batch = self._diffusion_scheduler.schedule_step()
        if step_batch is None:
            # 没有可执行的步，但仍可能存在 running 请求等待下一步（例如刚完成一批输出后）
            # 继续返回 False 以减少空转。
            return {}, False
        # 单步执行
        try:
            step_updates = self.model_executor.execute_diffusion_once(step_batch)  # type: ignore[attr-defined]
        except Exception as err:
            raise err

        # 合并更新并收集完成项
        finished_items = self._diffusion_scheduler.apply_updates_and_collect_finished(step_updates)

        # 按完成项打包输出（client 0 by default）
        from vllm.v1.engine import EngineCoreOutput, EngineCoreOutputs, FinishReason
        outputs: list[EngineCoreOutput] = []
        for item in finished_items:
            outputs.append(EngineCoreOutput(
                request_id=item.req_id,
                new_token_ids=list(item.tail_token_ids),
                finish_reason=FinishReason.LENGTH,
            ))
        if outputs:
            return ({0: EngineCoreOutputs(outputs=outputs)}, True)
        # 本步已执行但暂无完成输出
        return ({}, True)
...
class EngineCoreProc(EngineCore):
    ...
    def _process_input_queue(self):
        ...
        waited = False
        while not self.engines_running and not self._has_local_requests():
