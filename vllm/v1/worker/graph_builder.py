...
class MemoryAnalyzer:
    ...
    def _generate_optimized_reuse_plan(self) -> Dict[str, Dict[str, Any]]:
        ...
        for node in self.graph.execution_order:
            ...
            # ✨✨✨ 核心修正点 ✨✨✨
            # 当我们为 tensor_name 分配内存时，必须考虑所有在这一刻仍然存活的张量。
            # "存活"意味着这个张量的生命终点在当前张量的生命起点之后。
            current_birth_time = births[tensor_name]
            
            # 找出在 tensor_name 诞生时，所有仍然存活的内存块。
            # 这是通过比较生命周期来判断的。
            live_blocks_at_birth = []
            for block in allocated_blocks:
                block_name = block['tensor_name']
                # 如果一个已分配块的死亡时间 >= 当前块的出生时间，那么它们是冲突的，不能复用。
...
def define_llada_block_graph(P: int, O: int) -> CustomGraph:
    """
    ...
    Args:
        P: Prompt token 数量
        O: Output token 数量（需要计算logits的token）
    """
    ...
    # 只为 Output tokens 计算 logits
    graph.add_op('logits','get_logits',['final_output'],shape=(O,VOCAB_SIZE),dtype=DTYPE)

    logging.info("Manual graph construction complete.")
    return graph


def define_dream_block_graph(N: int, M: int) -> CustomGraph:
    """
    ...
    10. final_output -> lm_head -> logits (mask-only: 只对 M 个 tokens)
    """
    ...
    # 10. Logits 计算（lm_head）- 只为 M 个 mask tokens 计算 logits
    graph.add_op('logits', 'lm_head', ['final_output'], shape=(M, VOCAB_SIZE), dtype=DTYPE)
...
def define_dream_block_graph_chunkwise(
    ...
    # logits 只需要对 M 个 mask tokens 计算，所以基于 M 来分块
    max_chunk_size_logits = (M + num_chunks_logits - 1) // num_chunks_logits if chunk_logits else M
...
def define_llada_moe_block_graph(N: int, M: int) -> CustomGraph:
    """
    ...
    8. final_output -> lm_head -> logits (mask-only: 只对 M 个 tokens)
    ...
    # 11. Logits 计算（lm_head）- 只为 M 个 mask tokens 计算 logits
    graph.add_op('logits', 'lm_head', ['final_output'], 
                 shape=(M, VOCAB_SIZE), dtype=DTYPE)
...
def define_llada_block_graph_chunkwise(
    ...
    Args:
        P: Prompt token 数量
        O: Output token 数量（需要计算 logits 的 token）
        chunk_logits: 是否 chunk Logits
        chunk_mlp: 是否 chunk MLP
        num_chunks_logits: Logits 分块数
        num_chunks_mlp: MLP 分块数
    ...
    max_chunk_size_mlp = (N + num_chunks_mlp - 1) // num_chunks_mlp if chunk_mlp else N
    # logits 只需要对 O 个 tokens 计算，所以基于 O 来分块
    max_chunk_size_logits = (O + num_chunks_logits - 1) // num_chunks_logits if chunk_logits else O
...
if __name__ == "__main__":
    # 1. 使用你的函数定义图
    # P: Prompt tokens, O: Output tokens (需要计算 logits)
    # graph_blueprint = define_llada_block_graph(P=100000, O=10000)
    graph_blueprint = define_llada_block_graph_chunkwise(P=7, O=130000,num_chunks_logits=6,num_chunks_mlp=4)
