...
try:
    ...
    HAS_PULP = False
    logging.warning("PuLP not installed. ILP optimization will fall back to First-Fit. Install with: pip install pulp")
...
class MemoryAnalyzer:
    ...
    def _generate_optimized_reuse_plan(self) -> Dict[str, Dict[str, Any]]:
        ...
        if not HAS_PULP:
            raise ImportError(
                "PuLP library is required for ILP optimization. "
                "Install with: pip install pulp"
            )
        
        logging.info(f"Using ILP optimization for {len(self.graph.nodes)} tensors...")
        return self._generate_ilp_reuse_plan()

    def _generate_ilp_reuse_plan(self) -> Dict[str, Dict[str, Any]]:
        """
        使用整数线性规划（ILP）求解最优内存分配
        目标：最小化总内存占用
        约束：生命周期重叠的 tensor 不能在内存中重叠
        """
        ...
        # 变量1: 每个 tensor 的起始 offset（非负整数）
        offsets = {name: LpVariable(f"offset_{name}", lowBound=0, cat='Integer') 
                   for name in tensor_names}
        
        # 变量2: 总内存大小 M
        total_memory = LpVariable("total_memory", lowBound=0, cat='Integer')
        ...
        solver = PULP_CBC_CMD(
            ...
            options=[
                ...
                'randomCbcSeed 42',    # CBC特定随机种子
                'perturbation off',    # 关闭扰动
                'threads 1'            # 单线程（确保确定性）
            ]
        )
        prob.solve(solver)
        ...
        for name in tensor_names:
            offset_value = int(offsets[name].varValue) if offsets[name].varValue is not None else 0
        ...
        total_mem_value = int(total_memory.varValue) if total_memory.varValue is not None else 0
        reuse_plan['_total_buffer_size_bytes'] = total_mem_value
        
        logging.info(f"ILP solution found. Total memory: {total_mem_value / (1024**2):.2f} MB (status: {status})")
...
def define_dream_block_graph_chunkwise(
    ...
    if chunk_mlp:
        ...
        graph.add_op('xnorm2_barrier', 'barrier', ['x_normed_2'], shape=(0,), dtype=torch.int8)
    else:
        # 不 chunk MLP
        graph.add_op('x_gate', 'gate_proj', ['x_normed_2'], shape=(N, INTERMEDIATE_SIZE), dtype=DTYPE)
        graph.add_op('x_up', 'up_proj', ['x_normed_2'], shape=(N, INTERMEDIATE_SIZE), dtype=DTYPE)
