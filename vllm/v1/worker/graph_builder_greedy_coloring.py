...
class MemoryAnalyzer:
    ...
    def _generate_optimized_reuse_plan(self) -> Dict[str, Dict[str, Any]]:
        """
        Greedy Graph Coloring with DSATUR (Degree of Saturation) heuristic:
        1. Build conflict graph (edges between overlapping lifetimes)
        2. Iteratively select the tensor with highest saturation degree
        3. Assign it to the best available offset using Best-Fit
        4. This approach is based on register allocation algorithms
        """
        ...
        for i, name_i in enumerate(tensor_names):
            info_i = tensors_info[name_i]
            for name_j in tensor_names[i+1:]:
                info_j = tensors_info[name_j]
                # Check if lifetimes overlap
                if not (info_i['death'] < info_j['birth'] or info_j['death'] < info_i['birth']):
        ...
        uncolored = set(tensor_names)
        
        # Track which offsets each tensor's neighbors are using
        neighbor_offsets = {name: set() for name in tensor_names}
        
        while uncolored:
            # Select tensor with highest saturation degree
            # Saturation degree = number of different offsets used by neighbors
            # Tie-breaker: highest degree (number of conflicts)
            best_tensor = None
            best_sat_degree = -1
            best_conflict_degree = -1
            
            for tensor_name in uncolored:
                sat_degree = len(neighbor_offsets[tensor_name])
                conflict_degree = len(conflicts[tensor_name])
                
                if (sat_degree > best_sat_degree or 
                    (sat_degree == best_sat_degree and conflict_degree > best_conflict_degree)):
                    best_tensor = tensor_name
                    best_sat_degree = sat_degree
                    best_conflict_degree = conflict_degree
            
            # Allocate the selected tensor using Best-Fit
            tensor_name = best_tensor
            tensor_info = tensors_info[tensor_name]
            tensor_size = tensor_info['size']
            
            # Find all blocks that conflict with this tensor
            conflicting_blocks = []
            for block in allocated_blocks:
                if block['tensor_name'] in conflicts[tensor_name]:
                    conflicting_blocks.append(block)
            
            conflicting_blocks.sort(key=lambda b: b['start'])
            ...
            reuse_plan[tensor_name] = {
                'offset': best_offset,
                'size': tensor_size,
                'shape': tensor_info['spec'].shape,
                'dtype': tensor_info['spec'].dtype
            }
            allocated_blocks.append({
                'tensor_name': tensor_name,
                'start': best_offset,
                'end': best_offset + tensor_size
            })
            
            # Update neighbor offsets
            for neighbor in conflicts[tensor_name]:
                if neighbor in uncolored:
                    neighbor_offsets[neighbor].add(best_offset)
            
            uncolored.remove(tensor_name)
        ...
        max_offset = max((b['end'] for b in allocated_blocks), default=0)
        reuse_plan['_total_buffer_size_bytes'] = max_offset
        
        logging.info(f"Greedy Coloring (DSATUR) allocation: total memory = {max_offset / (1024**2):.2f} MB")
        return reuse_plan
