...
class MemoryAnalyzer:
    ...
    def _generate_optimized_reuse_plan(self) -> Dict[str, Dict[str, Any]]:
        ...
        # # === 修改开始 ===
        # solver = PULP_CBC_CMD(
        #     msg=1,            # 改为 1：打开求解器日志，看着它笨拙地搜索
        #     timeLimit=60,
        #     options=[
        #         'presolve off',      # 【核心垃圾化】关掉预处理：不让它简化问题，必须硬算
        #         'cuts off',          # 关掉割平面：不让它聪明地缩小搜索范围
        #         'heuristics off',    # 关掉启发式：不让它基于经验猜答案
        #         'preprocess off',    # 再次确保关掉预处理
        #         'strong branching off', # 关掉强分支策略
        #         'threads 1'          # 限制单线程，不让它并行加速
        #     ]
        # )
        # # === 修改结束 ===
        prob.solve(solver)
...
if __name__ == "__main__":
    # 1. 使用你的函数定义图
    # graph_blueprint = define_llada_block_graph(N=180000)
    graph_blueprint = define_llada_block_graph_chunkwise(N=200000,num_chunks_logits=4,num_chunks_mlp=9)
    ...
    print(f'analyze time = {(time.time() - st)*1000} ms')
