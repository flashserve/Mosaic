...
from vllm.v1.diffusion.types import DiffusionBatch, DiffusionStepBatch
...
class Worker(WorkerBase):
    ...
    def execute_diffusion(self, batch: DiffusionBatch):
            return self.model_runner.execute_diffusion(batch)

    def execute_diffusion_once(self, step_batch: DiffusionStepBatch):
            # return self.model_runner.execute_diffusion_once(step_batch)





        if not hasattr(self, '_diffusion_warmed_up'):
            print("🔥 Warming up diffusion pipeline...")
            warmup_start = torch.cuda.Event(enable_timing=True)
            warmup_end = torch.cuda.Event(enable_timing=True)
            
            warmup_start.record()
            # 执行一次warmup（不改变状态）
            _ = self.model_runner.execute_diffusion_once(step_batch)
            warmup_end.record()
            torch.cuda.synchronize()
            
            warmup_time = warmup_start.elapsed_time(warmup_end)
            print(f"✅ Warmup done in {warmup_time:.2f}ms (excluded from timing)")
            self._diffusion_warmed_up = True
        ...
        start_event.record()
        ret = self.model_runner.execute_diffusion_once(step_batch)
        end_event.record()
        torch.cuda.synchronize()
        print(f"execute_diffusion_once time = {start_event.elapsed_time(end_event)} ms")
        return ret
    ...
    def set_activation_scheduler_ref(self, scheduler) -> None:
        """为diffusion scheduler设置model runner引用，用于activation memory pool管理"""
        if hasattr(scheduler, 'set_model_runner'):
            scheduler.set_model_runner(self.model_runner)
            logger.info("[Worker] 已为diffusion scheduler设置model runner引用")
