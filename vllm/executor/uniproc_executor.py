...
class UniProcExecutor(ExecutorBase):
    ...
    def execute_diffusion(self, batch: DiffusionBatch):
        # 单进程执行：直接在 driver worker 上调用
        return self.collective_rpc("execute_diffusion", args=(batch, ))[0]

    def execute_diffusion_once(self, step_batch: DiffusionStepBatch):
        # 单步执行：直接在 driver worker 上调用
        return self.collective_rpc("execute_diffusion_once", args=(step_batch, ))[0]
