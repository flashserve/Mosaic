...
class SamplingParams(
    ...
    # -------------------- DLLM/Diffusion extra fields (custom) --------------------
    # Consumed by diffusion mode in EngineCore; ignored by standard decode.
    steps: int = 128
    gen_length: int = 128
    block_length: int = 32
    cfg_scale: float = 0.0
    ...
    mask_id: int = 126336
    #sbw
    token_step_schedule: Optional[list[int]] = None
    # For memory analysis: separate prompt and output tokens
    output_length: Optional[int] = None  # Number of output tokens (defaults to gen_length)
    prompt_length: Optional[int] = None  # Number of prompt tokens (will be set after tokenization)
    #-------------------------------------------------------------------------------
    ...
    def from_optional(
        ...
        return SamplingParams(
            ...
            # ======== 【2. 在返回值中添加这一行】 ========
            token_step_schedule=token_step_schedule,
            # ==========================================
        )
