...
def default_model_path() -> str:
    ...
    return os.environ.get("LLADA_MOE_MODEL_DIR",
                          str(Path(model_root) / "llada-moe-7b-a1b"))


def main() -> None:
    parser = argparse.ArgumentParser(description="LLaDA-MoE context benchmark")
    ...
    output_dir = args.output_dir or str(
        ...
        f"llada_moe_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    config = {
        "repo_root": args.project_path,
        "python_executable": args.python_executable,
        "conda_env": args.conda_env,
        "model_path": args.model_path,
        "model_env_var": "LLADA_MOE_MODEL_DIR",
