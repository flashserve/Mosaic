...
def default_model_path() -> str:
    ...
    return os.environ.get("DREAM_MODEL_DIR",
                          str(Path(model_root) / "dream-v0-instruct-7b"))


def main() -> None:
    parser = argparse.ArgumentParser(description="Dream context benchmark")
    ...
    output_dir = args.output_dir or str(
        ...
        f"dream_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    config = {
        "repo_root": args.project_path,
        "python_executable": args.python_executable,
        "conda_env": args.conda_env,
        "model_path": args.model_path,
        "model_env_var": "DREAM_MODEL_DIR",
