#!/usr/bin/env python3
"""
测试 flash_sample_low_confidence CUDA kernel
...
import time
import flash_sample_low_confidence

def pytorch_low_confidence_baseline(logits):
    """
    PyTorch 原生实现
    输入: logits [M, N] (bfloat16)
    输出: 
      - index [M]: argmax 索引
      - confidence [M]: argmax 位置的概率
    """
    # Argmax
    ...
    # Gather probability at argmax position
    x0_confidence = torch.gather(p_2d, dim=-1, index=x0_index.unsqueeze(-1)).squeeze(-1)  # [M]
...
def test_correctness(M, N, device='cuda'):
    ...
    idx_pt, conf_pt = pytorch_low_confidence_baseline(logits_pt)
    
    # CUDA kernel 实现
    logits_cuda = logits.clone().contiguous()
    idx_cuda, conf_cuda = flash_sample_low_confidence.low_confidence(logits_cuda)
...
def benchmark_performance(M, N, device='cuda', warmup=10, repeat=100):
    ...
    for _ in range(warmup):
        _ = pytorch_low_confidence_baseline(logits.clone())
        _ = flash_sample_low_confidence.low_confidence(logits.clone().contiguous())
...
def main():
    print("=" * 60)
    print("Flash Sample Low_Confidence Kernel 测试")
    print("=" * 60)
